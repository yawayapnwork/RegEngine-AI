...
JSONL fixtures, shaped like pipeline artifacts, so `build_dataset.py` and
`train_qlora.py` can be smoke-tested end-to-end without a live extraction run.
...
Synthetic fixtures generated by this script serve strictly as pipeline smoke tests and schema
validation fixtures; they do not constitute regulatory training data.
