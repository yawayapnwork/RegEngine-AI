...
4. **TODO / Roadmap: End-to-End Ingestion Velocity Benchmark (Target: <10 Minutes)**:
   ...
   - **Benchmark Specification Required to Turn Target into a Verified Claim**:
     - **Real SEBI Circular**: Evaluate on representative, multi-page SEBI master circulars (e.g., SEBI Master Circular for Stock Brokers) containing mixed tabular thresholds, entity scopes, and complex narrative obligations.
     - **Fixed Hardware/Model Configuration**: Benchmark on dedicated, pinned hardware (e.g., standardized GPU tier such as 8x A100/H100 or fixed Hugging Face Dedicated Endpoint tier) with standardized worker CPU/RAM instances and documented network conditions.
     - **Upload-to-Policy/HITL-Ready Timing**: Precise wall-clock measurement initiated at PDF ingestion and terminated upon writing compiled OPA Rego rules to the database in `AWAITING_HITL` status (excluding human reviewer think-time).
     - **Multiple Runs & Statistical Distribution**: Execute multiple runs ($N \ge 10$) across distinct documents to account for network variability, queue latency, and LLM token-generation variance; report **median** and **p95** percentiles.
     - **Reproducible Methodology**: Maintain an automated, open CLI harness (`python -m benchmarks.bench_circular_pipeline`) with deterministic seeds, pre-cached test PDFs, and exportable JSON timing artifacts.
